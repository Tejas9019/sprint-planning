...
class VectorStoreService:
    ...
    def add_source_document(
        self, source_id: str, user_id: str, filename: str, content: str
    ):
        logger.info(
            f"Adding source document {source_id} (user: {user_id}) to vector store..."
        )
        try:
            ...
            logger.info(
                f"Successfully indexed source document {source_id} in ChromaDB."
            )
        except Exception as e:
            logger.error(
                f"Failed to add document to vector store: {str(e)}", exc_info=True
            )
            raise e

    def delete_source_document(self, source_id: str, user_id: str):
        logger.info(
            f"Deleting source document {source_id} (user: {user_id}) from vector store..."
        )
        try:
            # Delete chunks matching source_id and user_id
            self.collection.delete(
                where={"$and": [{"source_id": source_id}, {"user_id": user_id}]}
            )
            logger.info(
                f"Successfully deleted source document {source_id} from vector store."
            )
        except Exception as e:
            logger.error(
                f"Failed to delete document from vector store: {str(e)}", exc_info=True
            )
            raise e

    def query_context(
        self, query: str, user_id: str, source_ids: List[str], top_k: int = 5
    ) -> List[str]:
        logger.info(
            f"Querying vector store for: '{query}' (user: {user_id}, sources: {source_ids})"
        )
        if not source_ids:
            return []
        try:
            ...
            filter_criteria = {
                "$and": [{"user_id": user_id}, {"source_id": {"$in": source_ids}}]
            }
            ...
            logger.info(
                f"Retrieved {len(documents)} relevant chunks from vector store."
            )
            return documents
        except Exception as e:
